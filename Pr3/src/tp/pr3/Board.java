...
public class Board {
	...
	public void load(BufferedReader in) throws IOException{
		...
		_boardSize = words.length;
		for(int j = 0; j < _boardSize; j++){
			_board[0][j].setValue(Integer.parseInt(words[j]));
		}
		
		for(int i = 1; i < _boardSize; i++){
			entrada = in.readLine();
			words = entrada.split("\\s+");
			for(int j = 0; j < words.length; j++)
				_board[i][j].setValue(Integer.parseInt(words[j]));
		}
	}
